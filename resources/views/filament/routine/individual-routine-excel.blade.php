...
<Worksheet ss:Name="Individual Routine">
<Table>
    {{-- Column widths --}}
    ...
    @php $totalCols = 2 + count($days) * count($timeSlots); @endphp
    <Row ss:Height="24">
        <Cell ss:StyleID="title" ss:MergeAcross="{{ $totalCols - 1 }}">
            <Data ss:Type="String">{{ $teacherName }}@if($teacherDept) — {{ $teacherDept }}@endif</Data>
        </Cell>
    </Row>
            ...
            @foreach($days as $dayIndex => $dayName)
                ...
                @foreach($timeSlots as $ts)
                    ...
                    <Cell ss:StyleID="{{ $styleId }}"@if($span > 1) ss:MergeAcross="{{ $span - 1 }}"@endif>
                        <Data ss:Type="String">{{ $cell ? $cell['course_code'] : '' }}</Data>
                    </Cell>
                @endforeach
            @endforeach
        </Row>
